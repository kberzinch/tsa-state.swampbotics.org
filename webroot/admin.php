...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    switch($_POST['action']) {
        case 'report-score':
            $name = getTeamName($_POST['team']);
            $number = strtoupper($_POST['team']);
            switch($_POST['type']) {
                case 'program':
                    $friendly = "Programming Skills";
                    $db->query(
                        'INSERT INTO scores SET vin=(?), program_score=(?), name=(?) ON DUPLICATE KEY UPDATE program_score=(?)',
                        'sisi',
                        $number,
                        $_POST['score'],
                        $name,
                ...
                case 'driver':
                    ...
                    $db->query(
                        'INSERT INTO scores SET vin=(?), driver_score=(?), name=(?) ON DUPLICATE KEY UPDATE driver_score=(?)',
                        'sisi',
                        $number,
                        $_POST['score'],
                        $name,
            ...
            $message = 'Your '.$friendly.' score of '.$_POST['score'].' for team '.$number.' ('.$name.') was successfully submitted.';
            break;
